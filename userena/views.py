...
@csrf_exempt
def payment(request, user_pk, para2):
    '''Change payment'''
    user = User.objects.get(pk=user_pk)
    us = UserenaSignup.objects.get(user=user)
    us.change_paid(True)
...
def payment_success(request):
    ...
    return render(request, 'userena/payment.html')

def group_payment(request):
    '''For group payments'''
    return render(request, 'userena/grouppayment.html')
...
def countdelegates(request):
    counter = len(UserenaSignup.objects.filter(paid=1))
    return HttpResponse("%d delegates have paid." % counter) 
